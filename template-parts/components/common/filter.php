    ...
    <form action="<?php echo esc_url(admin_url('admin-ajax.php')) ?>" method="POST" id="filter">
        <?php
            if( $sheen_terms = get_terms( array( 'taxonomy' => 'project_category', 'orderby' => 'name' ) ) ) : 
                echo sprintf( '<a class="c-filter__cat js-filter__cat active" href="%s">%s</a>' , esc_url(site_url()) , esc_html__( 'All', 'sheen' ) );
                foreach ( $sheen_terms as $sheen_term ) :
                    ...
                    echo sprintf( '<label tabindex="0" class="c-filter__cat js-filter__cat" for="c-%s">%s</label>' , esc_attr($sheen_term->term_id) , esc_html($sheen_term->name));
                endforeach;
            endif;
        ?>
